...
import cn.edu.tsinghua.iotdb.kairosdb.aggregator.Sampling;
...
public class Util {
  ...
  /**
   * Returns the host name. First tries to execute "hostname" on the machine. This should work for
   * Linux, Windows, and Mac. If, for some reason hostname fails, then get the name from InetAddress
   * (which might change depending on network setup)
   *
   * @return hostname
   */
  ...
  /**
   * Returns true if the string contains a number. This means it contains only digits, the minus
   * sign, plus sign and a period.
   *
