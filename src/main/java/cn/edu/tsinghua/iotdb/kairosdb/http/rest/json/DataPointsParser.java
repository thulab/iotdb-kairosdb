...
public class DataPointsParser {
    ...
    public int getIngestTime() { return ingestTime; }
    ...
    private boolean validateAndAddDataPoints(NewMetric metric, ValidationErrors errors, int count) {
        ...
        if (!validationErrors.hasErrors())
        {
            ...
            if (metric.getTimestamp() != null && metric.getValue() != null) {
                ...
                try {
                    MetricsManager.addDatapoint(metric.getName(), tags, type, metric.getTimestamp(), metric.getValue().getAsString());
            ...
            if (metric.getDatapoints() != null && metric.getDatapoints().length > 0)
            {
                ...
                for (JsonElement[] dataPoint : metric.getDatapoints()) {
                    ...
                    else
                    {
                        ...
                        if (!Validator.isNotNull(validationErrors, dataPointContext.setAttribute("timestamp"), timestamp))
                            continue;
                        ...
                        try {
                            MetricsManager.addDatapoint(metric.getName(), tags, type, timestamp, dataPoint[1].getAsString());
                        } catch (SQLException e) {
                            validationErrors.addErrorMessage(context + " " + e.getMessage());
                        }
    ...
    private static class NewMetric {
        ...
        public Long getTimestamp() { if (time != null) return time; else return timestamp; }

        public JsonElement getValue() { return value; }

        public Map<String, String> getTags() { return tags; }

        public JsonElement[][] getDatapoints() { return datapoints; }
